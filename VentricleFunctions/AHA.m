...
%     figure(i); plotthree(nodes(layer{i}, :), '.');
    xlim([-40 40]); ylim([-50 50]); zlim([0 60]); axis image;view(3)
end
i = length(divisions);
...
figure; hold on
map = colormap('parula');
subdomain_map = map(floor(linspace(1, length(map), max(unique(lst)))), :);
clear l
for j = 1:max(unique(lst))
        ...
        trisurf(subbdry{j}, pts(find(lst == j), 1), ...
            pts(find(lst == j), 2),...
            pts(find(lst == j), 3),...
             'facecolor', subdomain_map(j, :), 'edgecolor', subdomain_map(j, :)*0.8);
         
             l(j) = string(sprintf('subdomain %i', j));

end
axis image off; view(3); legend(l)
...
for i = unique(p)
    ...
    trisurf(jtri{i}, joined{i}(:, 1), joined{i}(:, 2), joined{i}(:, 3),...
        'facecolor', subdomain_map(i, :), 'edgecolor', subdomain_map(i, :)*0.8);
    hold on
