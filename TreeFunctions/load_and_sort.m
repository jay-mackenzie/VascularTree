function [seg, inlet, bifNodes, endpaths, endNodes, connectedEdges] = load_and_sort()
% path = struct2cell(load("path.mat")); path = path{1};
path = load('path.mat'); path= path.path;
...
if max(ListAllIDs) == length(ListAllIDs); printf("still got disconnects"); end
